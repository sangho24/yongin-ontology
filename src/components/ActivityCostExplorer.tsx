...
import activityCostData from "@/data/activity_cost.json";
import type {
  ZoneGroup,
  DetailZone,
  ActivityCostBlock,
...
// =============================================================================
// ActivityCostExplorer — 구역·기간 선택 → 매출/비용/항목 + lineage 통합 패널
// CEO·부서장이 매일 켜서 쓰는 운영 BI의 핵심 인터랙션
// =============================================================================

interface ActivityCostExplorerProps {
  side?: ZoneVcSide | "all";
...
const ZONE_GROUPS = activityCostData.zoneGroups as unknown as ZoneGroup[];
const ACTIVITY_COSTS = activityCostData.activityCosts as unknown as ActivityCostBlock[];
const LINEAGES = activityCostData.lineages as unknown as Record<string, NumberLineage>;

const ALL_ZONES_MAP = new Map<string, DetailZone>();
ZONE_GROUPS.forEach((g) => g.subZones.forEach((z) => ALL_ZONES_MAP.set(z.id, z)));
  ...
  blocks.forEach((b) => {
    b.items.forEach((item) => {
      ...
      if (!existing) {
        ...
        existing.zoneIds.push(b.zoneId);
        // 다중일 때는 단일 lineage 사용 X — 자동 lineage 빌드
...
// 통합 항목에 대한 lineage 자동 구성 (primaryLineage 없을 때)
function buildLineage(item: AggregatedItem): NumberLineage {
  if (item.primaryLineage) return item.primaryLineage;
  const zoneLabels = item.zoneIds
    .map((id) => ALL_ZONES_MAP.get(id)?.label ?? id)
    .join(", ");
  ...
  return {
    source:
      item.zoneIds.length === 1
        ? `${zoneLabels} / ${item.account}`
        : `${item.zoneIds.length}개 구역 / ${item.account}`,
    formula: isRevenue
      ? `SUMIFS(원장, 계정=${item.account}, 구역∈{${zoneLabels}})`
    ...
    steps: [
      {
        label: item.zoneIds.length === 1 ? "원장 raw 추출" : `${item.zoneIds.length}개 구역 합산`,
      },
      { label: "마감분개·이월 제외" },
...
export function ActivityCostExplorer({
  side = "all",
  defaultZoneIds = [],
  title = "활동원가 탐색기",
  ...
  return (
    ...
    >
      ...
      {/* Controls — zone selector + period toggle */}
      <div className="grid gap-6 px-8 py-6 lg:grid-cols-[1fr_auto] lg:items-start">
        <ZoneSelector
          zoneGroups={filteredGroups}
          selected={selectedIds}
          onChange={setSelectedIds}
          side={side}
          placeholder="구역을 선택해주세요 (다중 선택 가능)"
        />
      ...
      {selectedIds.length === 0 ? (
        ...
        <div className="px-8 pb-10 fade-in">
          ...
          <div className="grid gap-10 pt-2 lg:grid-cols-2">
            ...
            <ItemColumn
              icon={
                <TrendingDown className="h-4 w-4 text-[#9a3412]" strokeWidth={1.75} />
              }
              title="비용"
              items={costs}
              period={period}
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
      ...
      <div className="text-[13px] leading-relaxed text-stone-500">
        구역을 선택하면 매출·비용·항목과 lineage가 펼쳐집니다
      </div>
...
function ItemRow({ item, period }: { item: AggregatedItem; period: Period }) {
  ...
  // 우선순위: estimated > allocated > ledger (가장 약한 신호 표기)
